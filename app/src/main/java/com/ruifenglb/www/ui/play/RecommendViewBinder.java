...
public class RecommendViewBinder extends ItemViewBinder<RecommendBean, RecommendViewBinder.ViewHolder> {
    ...
    class LastestAdapter extends BaseQuickAdapter<VodBean, BaseViewHolder> {
        ...
        public LastestAdapter(List<VodBean> data) {
            super(R.layout.item_card_recommend_child,data);
        }
        ...
        protected void convert(@NonNull BaseViewHolder helper, VodBean item) {
            ...
            if (item.getVod_points_play() == 0) {
                helper.getView(R.id.item_tv_card_child_tip).setVisibility(View.GONE);
            } else {
                helper.getView(R.id.item_tv_card_child_tip).setVisibility(View.VISIBLE);

                helper.setText(R.id.item_tv_card_child_tip, item.getVod_points_play() + "积分");
